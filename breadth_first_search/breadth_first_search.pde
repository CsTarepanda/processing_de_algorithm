...
void mousePressed() {
  if (end || click) return;
  click = true;
  Deque<Lambda> queue = new ArrayDeque<Lambda>();
  top.resolve(queue);
  while(queue.size() != 0 && !end) queue.poll().apply();
  que.push(new Lambda() {
    Node n;
    public void apply() {
      n.changeColor(CLEAR);
...
class Tree {
  ...
  void resolve(Deque<Lambda> queue) {
    que.add(new Lambda() {
      Node n;
      public void apply() {
        n.changeColor(CLEAR);
...
class Node {
  ...
  void resolve(final Deque<Lambda> queue) {
    if (end) return;
    if (l != null) queue.add(new Lambda(){
      public void apply(){
        l.resolve(queue);
      }
    });
    if (end) return;
    if (r != null)  queue.add(new Lambda(){
      public void apply(){
        r.resolve(queue);
      }
    });
